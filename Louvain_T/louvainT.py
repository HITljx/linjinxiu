...
class PyLouvain:
    ...
    def apply_method(self):
        ...
        for communID in range(len(self.actual_partition)):
            #i是社区号：0-16
            print "+++++++++++++++++++++++self.actual_partition---" + str(communID) + " +++++++++++++++++++++++++++++++++"
            #一个社区所有的ID
            #print self.actual_partition[communID]
            for j in self.actual_partition[communID]:
                #print j
                id_domain_sql = "select siteID,siteDomain,tags from Tags_simple where siteID = "+ str(j)
                cur.execute(id_domain_sql)
                id_records = cur.fetchall() 
                for i in id_records:
                    #print i
                    #插入community：siteID，siteDomain，tags，commmunityID
                    item_value = []
                    item_value = list(i)
                    item_value.append(str(communID))
                    print item_value
                    insert_sql = "insert into community(siteID,siteDomain,tags,CommunID)" + "values(%s,%s,%s,%s)"
                    #insert_sql = "insert into community(siteID,siteDomain,tags)" + "values(%s,%s,%s)"
